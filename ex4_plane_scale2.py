...
SegNet_OUTPUT_HEIGHT = 224
SegNet_OUTPUT_WIDTH = 304
#
...
class Model:
    ...
    def get_scale2_on_RGB_label_net_model(self):
        input_tensor1 = Input(shape=(IMAGE_HEIGHT, IMAGE_WIDTH, 3), name='image_input1')
        input_tensor2 = Input(shape=(TARGET_HEIGHT, TARGET_WIDTH, 1), name='predict_input1')
        input_tensor3 = Input(shape=(TARGET_HEIGHT, TARGET_WIDTH, LABEL_NUMBER), name='label_input1')
        #
        #
        x = Conv2D(filters=63, kernel_size=(9, 9), strides=2,
                   name='image_conv1', kernel_initializer='he_normal')(input_tensor1)
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as session:
        ...
        history_all = np.zeros((SCALE_ALL, EPOCHS + 1, 17))
        ### sstage
        data_idx = np.arange(images.shape[0])
        count_train = np.zeros((SCALE_ALL, EPOCHS + 1, 17))
        for for_stage in range(STAGE_ALL):
            ...
            for for_scale in range(SCALE_ALL):
                ...
                if for_scale == 1:
                    ...
                    scale2_model = ins_model.get_scale2_on_RGB_label_net_model()
                    ...
                    scale2_model.compile(optimizer=OPTIMIZER_SCALE2,
                                         ...
                                         metrics=['mean_absolute_error', 'mean_squared_error',
                                                  ...
                                                  thresholded_accuracy2,
                                                  thresholded_accuracy3])
                history1 = np.zeros((1, EPOCHS + 1, 17))
                ...
                history1[0, 0, 9:17] = predict_loss
                ### eepoch
                for for_epoch in range(EPOCHS):
                    ...
                    history1[0, for_epoch + 1, 1:9] = predict_loss
                    #
                    ### test batch
                    predict_loss = np.zeros(8)
                    for for_1 in range(test_per_sample):
                    ...
                    predict_loss = predict_loss / test_per_sample
                    history1[0, for_epoch + 1, 9:17] = predict_loss
                    # predict
