...
if args.scale2:
    ...
    scale2_base_weights_path = path_the_directory + 'weights/' + \
                               'save_ex2_one-hot_simple5_scale2_scale2'
# limit
if args.limit:
            ...
            for for_scale in range(SCALE_ALL):
                ...
                for index_number in range(batch_per_sample):
                    ...
                    if for_scale == 1:
                        ...
                        predict_overlap = SegNet_model.predict([get_x_train, predict_depth])
                        ...
                        predict_overlap = np_utils.to_categorical(predict_overlap, OVERLAP_NUMBER)
                        predict_loss = predict_loss + \
                ...
                for for_1 in range(test_per_sample):
                    ...
                    if for_scale == 1:
                        ...
                        predict_overlap = SegNet_model.predict([x_test[select_sep1:select_sep2], predict_depth])
                        predict_overlap = np.reshape(predict_overlap, (
                            predict_overlap.shape[0], SegNet_OUTPUT_HEIGHT, SegNet_OUTPUT_WIDTH, OVERLAP_NUMBER))
                        predict_overlap = np.amax(predict_overlap, axis=-1)
                        predict_overlap = np.reshape(predict_overlap,
