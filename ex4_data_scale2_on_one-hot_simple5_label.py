...
TRAIN_FILE_PATH_SELECT = 'imageDepthOverlap_simple5.csv'
...
if args.seg:
    ...
    seg_base_weights_path = path_the_directory + 'weights/' + \
                               'save_ex4_one-hot_simple5_scale2_scale2'
# scale2 weight path
if args.scale2:
    ...
    scale2_base_weights_path = path_the_directory + 'weights/' + \
                               'save_ex4_scale2_on_one-hot_simple5_label_scale2'
# limit
if args.limit:
    ...
    def coco_load(self, file_path, arg_size=(IMAGE_HEIGHT, IMAGE_WIDTH)):
        ...
        for get_subset in ["train", "minival"]:
            ...
            for i_1, image_id in enumerate(get_ids):
                ...
                list_label1.append(label_array)
                ...
                list_label2.append(image_array2)
                print("\r{0:d}\t{0:d}".format(i_1, image_id), end="")
                mask_count += 1
        list_img = np.array(list_img) / 255.0
        list_img = list_img.astype('float32')
        list_label1 = np.array(list_label1)
        list_label2 = np.array(list_label2)
        list_label1 = list_label1.astype('float32')
        list_label2 = list_label2.astype('float32')
        print("")
        print("### img.shape\t{}".format(list_img.shape))
        print("### label1.shape\t{}".format(list_label1.shape))
        print("### label2.shape\t{}".format(list_label2.shape))
        return list_img, list_label1, list_label2
...
class Model:
    ...
    def get_SegNet_model(self, classes=OVERLAP_NUMBER):
        ...
        input_tensor3 = Input(shape=(IMAGE_HEIGHT, IMAGE_WIDTH, LABEL_NUMBER), name='label_input1')
        #
        y = UpSampling2D((4, 4), name='upsampling1')(input_tensor2)
        y = ZeroPadding2D(padding=((4, 4), (4, 4)))(y)
        x = Concatenate(axis=-1)([input_tensor1, y, input_tensor3])
        #
        insep_1 = Conv2D(filters=64, kernel_size=(1, 1), padding='same',
                         name='insep1_conv1', kernel_initializer='he_normal')(x)
        insep_1 = BatchNormalization()(insep_1)
        ...
        SegNet_prediction = x
        SegNet_model = keras.models.Model(inputs=[input_tensor1, input_tensor2, input_tensor3],
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as session:
        ...
        images, labels1, labels2= ins_data_set.coco_load(file_path=COCO_DIR)
        ...
        print("### depths{}\toverlaps{}\n### images{}\tlabels1{}\tlabels2{}".format(
            depths1.shape, overlaps.shape, images.shape, labels1.shape, labels2.shape
        ))
        # instance
        ins_model = Model()
        ...
        SegNet_model.compile(optimizer='adadelta',
                             ...
                             metrics=['mean_absolute_error', 'mean_squared_error',
                                      ...
                                      'accuracy'])
        # scale2
        scale2_model = ins_model.get_scale2_on_RGB_one_hot_overlap_net_model()
        ...
        for for_img in range(images.shape[0]):
                ...
                set_label = np_utils.to_categorical(labels1[for_img:for_img + 1], LABEL_NUMBER)
                ...
                for get_label in range(LABEL_NUMBER):
                    label_pos = labels2[for_img] == get_label
                ...
                ins_predict_to_html.convert_on_RGB_label_overlap_predict2 \
                    (images[for_img], labels2[for_img], overlaps[for_img], predict_depth2[0], depths2[for_img],
